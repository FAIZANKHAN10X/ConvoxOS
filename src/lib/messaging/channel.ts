...
// ------------------------------------------------------------
// WhatsApp Meta sender (interactive variants).
//
// Shared service-role sender used by CRM messaging paths (sequences,
// broadcasts). Kept separate so provider API + phone-variant retry +
// DB persistence stay in one place.
// ------------------------------------------------------------
...
/**
 * Send a plain-text WhatsApp message (service-role).
 *
 * Used by CRM messaging paths (sequences, AI auto-reply) that prompt
 * the customer with text.
 *
 ...
 * base once template-variable and media sends settle.
 */
...
/**
 * Send an interactive-button WhatsApp message (service-role).
 *
 ...
 *
 * Returns the Meta message id so the caller can stash it for later
 * reference (e.g. reply matching).
 */
...
/**
 * Send an interactive-list WhatsApp message (service-role).
 * Used when more than 3 options are needed (Meta's button cap).
 */
...
async function sendInteractiveViaMeta(
  ...
  // defense-in-depth: a caller must never send through another
  // account's number. Migration 017 moved both tables to
  ...
  // Meta's sandbox quirks all need this to reliably land a message.
  const variants = phoneVariants(sanitized)
  let workingPhone = sanitized
  let waMessageId = ''
  let lastError: unknown = null

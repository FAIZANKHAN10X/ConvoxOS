    ...
    <Card className={isExpanded ? 'ring-1 ring-primary/20' : undefined}>
      <CardHeader className="flex-row items-center justify-between gap-3 space-y-0">
        <div className="flex items-center gap-3">
          <span
            className={`flex size-9 items-center justify-center rounded-lg ${id === 'whatsapp' ? 'bg-emerald-500/15 text-emerald-500' : 'bg-sky-500/15 text-sky-500'}`}
          >
            {id === 'whatsapp' ? <MessageCircle className="size-5" /> : <Send className="size-5" />}
          </span>
          <div>
            <CardTitle className="text-sm">
              {id === 'whatsapp' ? t('whatsappTitle') : t('telegramTitle')}
            </CardTitle>
            <CardDescription className="text-xs">
              {id === 'whatsapp' ? t('whatsappDesc') : t('telegramDesc')}
            </CardDescription>
          </div>
        </div>

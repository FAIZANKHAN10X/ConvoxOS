...
export function ContactsClient({ initialContacts, initialTotalCount, initialTags }: ContactsClientProps) {
  ...
  // All tags for display — seeded from server to avoid first waterfall
  const [tagsMap, setTagsMap] = useState<Record<string, Tag>>(() => {
    const map: Record<string, Tag> = {}
    initialTags.forEach((tag) => (map[tag.id] = tag))
    return map
  })
  ...
  // Track server hydration — skip redundant first fetches when page
  // is on its initial state (page 0, no search, no tag filter) and
  // the server already supplied contacts + tags.
  const hydratedRef = useRef(initialContacts.length > 0)
  ...
  useEffect(() => {
    if (hydratedRef.current && Object.keys(tagsMap).length > 0) return
    // eslint-disable-next-line react-hooks/set-state-in-effect
    fetchTags();
  }, [fetchTags, tagsMap]);

  useEffect(() => {
    if (hydratedRef.current && page === 0 && search === '' && selectedTagIds.length === 0) {

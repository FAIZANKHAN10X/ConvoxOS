...
export default function PipelinesClient({
  ...
  // Initial load + seed-if-empty. Skipped when the server already
  // supplied pipelines — stages/deals still load via the selection
  // effect below. Empty initial state keeps the legacy seed flow.
  useEffect(() => {
    let cancelled = false;
    (async () => {
      setLoading(true);
      let list = initialPipelines.length > 0 ? initialPipelines : await loadPipelines();
    ...
    // initialPipelines is server-supplied once per navigation; listing
    // it keeps the lint rule honest without refiring in practice.
  }, [loadPipelines, seedDefaultPipeline, initialPipelines]);
  ...
  const handleDealMoved = useCallback(
    async (dealId: string, newStageId: string) => {
      ...
      // Goes through the deals API route (not a direct write) so the
      // move emits `deal_stage_changed` for automations.
      setDeals((prev) =>

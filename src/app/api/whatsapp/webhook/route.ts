...
async function processMessage(
  ...
  // WhatsApp-specific normalization (provider edge) — keep Meta media verification
  // and content-type mapping here per approved boundary; shared processor owns
  // provider-independent CRM steps.

  // Reaction envelope is handled by shared via kind, but skip media fetch
  if (message.type === 'reaction') {
    const senderPhone = normalizePhone(message.from)
    // Delegate reaction via shared (kind=reaction, target + emoji)
  ...
  const { contentText, mediaUrl, mediaType, interactiveReplyId } =
    await parseMessageContent(message, accessToken, mirrorMedia ? { accountId } : null)
  ...
  const senderPhone = normalizePhone(message.from)
  const replyToProviderId: string | null = message.context?.id ?? null

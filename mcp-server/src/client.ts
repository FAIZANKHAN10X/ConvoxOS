...
/** A structured error from the ConvoxOS API envelope (`{ error: { code, message } }`). */
  ...
  private async request<T>(
    ...
    try {
      ...
      throw new WacrmApiError(
        0,
        'network_error',
        `Could not reach ConvoxOS at ${this.baseUrl}: ${(err as Error).message}`,
      );
    }
